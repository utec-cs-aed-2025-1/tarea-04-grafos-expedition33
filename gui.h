...
class GUI
{
    ...
    std::string status_msg = "Selecciona Start y End";

    void set_status(const std::string &msg)
    {
        status_msg = msg;
        status_text.setString(status_msg);
    }
...
public:
        ...
        : path_finding_manager(&window_manager), graph(&window_manager)
    {
        ...
        // Cargar fuente (archivo debe estar en la raíz)
        if (!font.loadFromFile("arial.ttf"))
        {
            std::cerr << "No se pudo cargar arial.ttf desde: "
        ...
        instructions_text.setFillColor(sf::Color::Yellow);
        instructions_text.setCharacterSize(15);
        instructions_text.setPosition(10, 600);
        instructions_text.setString(
            "Controles:\n"
            "Click 1 = Selecciona Start\n"
            "Click 2 = Selecciona End\n"
            "D = Ejecutar Dijkstra\n"
            "A = Ejecutar A*\n"
            "B = Ejecutar Best First Search\n"
            "E = Mostrar aristas visitadas\n"
            "R = Reset\n"
            "Q = Salir\n");
    }
        ...
        while (window_manager.is_open())
        {
            ...
            while (window_manager.poll_event(event))
            {
                ...
                case sf::Event::KeyPressed:
                {
                    ...
                    case sf::Keyboard::D:
                    {
                        if (path_finding_manager.src && path_finding_manager.dest)
                        {
                            set_status("Ejecutando Dijkstra...");
                            path_finding_manager.exec(graph, Dijkstra);
                            set_status("Dijkstra ejecutado");
                        }
                        else
                        {
                            set_status("Selecciona Start y End primero");
                        }
                        break;
                    }
                    // A = Ejecutar AStar
                    case sf::Keyboard::A:
                    {
                        if (path_finding_manager.src && path_finding_manager.dest)
                        {
                            set_status("Ejecutando A*...");
                            path_finding_manager.exec(graph, AStar);
                            set_status("A* ejecutado");
                        }
                        else
                        {
                            set_status("Selecciona Start y End primero");
                    ...
                    case sf::Keyboard::B:
                    {
                        if (path_finding_manager.src && path_finding_manager.dest)
                        {
                            set_status("Ejecutando Best First Search...");
                            path_finding_manager.exec(graph, BestFS);
                            set_status("Best First Search ejecutado");
                        }
                        else
                        {
                            set_status("Selecciona Start y End primero");
                    ...
                    case sf::Keyboard::E:
                    {
                        ...
                        set_status(draw_extra_lines ? "Mostrando aristas visitadas" : "Aristas visitadas ocultas");
                        break;
                    }
            ...
            // Dibujar texto
            window_manager.get_window().draw(status_text);
            window_manager.get_window().draw(instructions_text);
